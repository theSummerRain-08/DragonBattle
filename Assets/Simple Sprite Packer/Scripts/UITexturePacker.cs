...
public class UITexturePacker
{
	...
	{
		int area = 0;
		// Debug.Log("compute sum total pixel of all textures");
	...
	public static Rect[] PackTextures (Texture2D texture, Texture2D[] textures, int width, int height, int padding, int maxSize)
	{
		...
		// khoi tao texture voi kich thuoc moi - sau do xet luong pixel cho texture
		texture.Reinitialize(width, height);
		texture.SetPixels(new Color[width * height]);
		
		
		// The returned rects
		// lap qua tung texture in textures, sau do gan pixel cua tung textures do cho texture chinh.
	...
	Rect FindPositionForNewNodeBestAreaFit (int width, int height, ref int bestAreaFit, ref int bestShortSideFit)
	{
		...
		// Debug.Log("bestnode:");
		// Debug.Log(bestNode.x + " - " + bestNode.y);
		// Debug.Log(bestNode.width + " - " + bestNode.height);
