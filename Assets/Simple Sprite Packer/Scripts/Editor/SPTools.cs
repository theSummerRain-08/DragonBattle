...
namespace SimpleSpritePackerEditor
{
	public static class SPTools
	{
		...
		public static void PrepareDefaultEditorPrefs()
		{
			...
			if (!EditorPrefs.HasKey(Settings_AllowMuliSpritesOneSource))
			{
				EditorPrefs.SetBool(Settings_AllowMuliSpritesOneSource, true);
			}
		}
		...
		/// <param name="path">Path.</param>
		private static bool RemoveReadOnlyFlag(string path)
		{
		...
		public static bool TextureSetReadWriteEnabled(Texture2D texture, bool enabled, bool force)
		{
			...
			return AssetSetReadWriteEnabled(assetPath, enabled, force);
		}
		...
		public static bool AssetSetReadWriteEnabled(string path, bool enabled, bool force)
		{
			...
			TextureImporterSettings settings = new TextureImporterSettings();
			// doc setting cua asset o path toi settings
		...
		public static bool ImportAndConfigureAtlasTexture(Texture2D targetTexture, Texture2D sourceTexture, Rect[] uvs, SPSpriteImportData[] spritesImportData)
		{
			...
			List<SpriteMetaData> spritesheetMetaList = new List<SpriteMetaData>();
			...
			for (int i = 0; i < uvs.Length; i++)
			{
				if (HasSpritesheetMeta(texImporter.spritesheet, spritesImportData[i].name))
				{
					...
					spritesheetMetaList.Add(currentMeta);
				}
				else
				{
			...
			updateSprites(texImporter, spritesheetMeta);
#else
            texImporter.spritesheet = spritesheetMetaList.ToArray();
#endif
			texImporter.spritesheet = spritesheetMetaList.ToArray();
			...
			// DoAssetReimport(assetPath, ImportAssetOptions.ForceUpdate);
		...
		public static Object[] FilterResourcesForAtlasImport(Object[] resources)
		{
			...
			foreach (Object resource in resources)
			{
				...
				{
					Object[] subAssets = FilterResourcesForAtlasImport(GetSubAssets(resource));
				...
				{
					Object[] subAssets = FilterResourcesForAtlasImport(GetDirectoryAssets(resourcePath));
        ...
        private static void updateSprites(TextureImporter importer, SpriteMetaData[] metaDatas)
        {
	        var dataProvider = GetSpriteEditorDataProvider(importer);
	        var spriteNameFileIdDataProvider = dataProvider.GetDataProvider<ISpriteNameFileIdDataProvider>();

	        var oldIds = spriteNameFileIdDataProvider.GetNameFileIdPairs();
	        SpriteRect[] rects = spriteMetadataToSpriteRects(metaDatas);
	        SpriteNameFileIdPair[] ids = generateSpriteIds(oldIds, rects);
        ...
        private static SpriteRect[] spriteMetadataToSpriteRects(SpriteMetaData[] metadata)
        {
